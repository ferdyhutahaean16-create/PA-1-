            ...
            <form action="{{ route('curriculum.update', $curriculum->id) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="kode_mk" value="{{ $curriculum->kode_mk }}" class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" required>
                    </div>
                </div>

                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Nama Mata Kuliah <span class="text-red-500">*</span></label>
                    <input type="text" name="mata_kuliah" value="{{ $curriculum->mata_kuliah }}" class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" required>
                </div>

                <div class="w-full md:w-1/2">
                    <label class="block text-sm font-bold text-gray-700 mb-2">Jumlah SKS <span class="text-red-500">*</span></label>
                    <input type="number" name="sks" value="{{ $curriculum->sks }}" min="1" max="6" class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" required>
                </div>
