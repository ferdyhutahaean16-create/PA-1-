    ...
    <div class="container mx-auto px-6 max-w-4xl">
        <div class="mb-8 flex items-center justify-between">
            <div>
                ...
                <p class="text-gray-500">Perbarui data atau foto untuk {{ $lab->nama_lab }}.</p>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
            <form action="{{ route('laboratorium.update', $lab->id) }}" method="POST" enctype="multipart/form-data" class="p-8">
                @csrf
                @method('PUT') <div class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="nama_lab" value="{{ $lab->nama_lab }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-[#1a4a38]" required>
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Kepala Laboratorium</label>
                            <input type="text" name="kepala_lab" value="{{ $lab->kepala_lab }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-[#1a4a38]">
                        </div>
                    </div>

                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">Fasilitas Utama</label>
                        <input type="text" name="fasilitas" value="{{ $lab->fasilitas }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-[#1a4a38]">
                    </div>

                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">Deskripsi Ruangan</label>
                        <textarea name="deskripsi" rows="4" class="ckeditor-field w-full border-gray-300 rounded-lg shadow-sm focus:ring-[#1a4a38]">{{ $lab->deskripsi }}</textarea>
                    </div>
                    ...
                    <div class="pt-6 border-t border-gray-100">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="border border-gray-200 p-4 rounded-lg bg-gray-50 flex items-center gap-4">
                                @if($lab->foto)
                                    <img src="{{ asset($lab->foto) }}" class="w-16 h-16 object-cover rounded shadow-sm border border-gray-200">
                                @else
                            ...
                            <div class="border border-gray-200 p-4 rounded-lg bg-gray-50 flex items-center gap-4">
                                @if($lab->foto_2)
                                    <img src="{{ asset($lab->foto_2) }}" class="w-16 h-16 object-cover rounded shadow-sm border border-gray-200">
                                @else
                                    <div class="w-16 h-16 bg-gray-200 rounded flex items-center justify-center text-xs text-gray-400">Kosong</div>
                                @endif
                                <div class="flex-1">
                            ...
                            <div class="border border-gray-200 p-4 rounded-lg bg-gray-50 flex items-center gap-4">
                                @if($lab->foto_3)
                                    <img src="{{ asset($lab->foto_3) }}" class="w-16 h-16 object-cover rounded shadow-sm border border-gray-200">
                                @else
                                    <div class="w-16 h-16 bg-gray-200 rounded flex items-center justify-center text-xs text-gray-400">Kosong</div>
                                @endif
                                <div class="flex-1">
                            ...
                            <div class="border border-gray-200 p-4 rounded-lg bg-gray-50 flex items-center gap-4">
                                @if($lab->foto_4)
                                    <img src="{{ asset($lab->foto_4) }}" class="w-16 h-16 object-cover rounded shadow-sm border border-gray-200">
                                @else
                                    <div class="w-16 h-16 bg-gray-200 rounded flex items-center justify-center text-xs text-gray-400">Kosong</div>
                                @endif
                                <div class="flex-1">
                ...
                <div class="mt-8 flex justify-end">
                    <button type="submit" class="bg-yellow-500 text-white px-8 py-3 rounded-lg font-bold shadow hover:bg-yellow-600 transition">Update Laboratorium</button>
                </div>
            </form>
        </div>
    </div>
