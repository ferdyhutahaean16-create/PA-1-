            ...
            <form action="{{ route('lecturer.store') }}" method="POST" enctype="multipart/form-data">
                @if ($errors->any())
                    <div class="bg-red-50 border-l-4 border-red-500 text-red-700 p-4 mb-6" role="alert">
                        <p class="font-bold">Gagal Menyimpan Data Dosen!</p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-8">
                    ...
                    <div>
                        <label for="name" class="block text-sm font-semibold text-gray-600 mb-2">Nama Lengkap & Gelar</label>
                        <input type="text" name="name" id="name" placeholder="Contoh: Dr. Eng. John Doe, M.T." class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-biotech-secondary/50 focus:border-biotech-secondary transition" required>
                    </div>
                    ...
                    <div>
                        <label for="position" class="block text-sm font-semibold text-gray-600 mb-2">Posisi (Jabatan)</label>
                        <input type="text" name="position" id="position" placeholder="Contoh: Dosen Pengajar / Asisten Ahli" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-biotech-secondary/50 focus:border-biotech-secondary transition" required>
                    </div>

                    <div>
                        <label for="education" class="block text-sm font-semibold text-gray-600 mb-2">Latar Belakang Pendidikan</label>
                        <input type="text" name="education" id="education" placeholder="Contoh: S1 IT Del, S2 ITB, S3 Tohoku Univ." class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-biotech-secondary/50 focus:border-biotech-secondary transition" required>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="phone_number" id="phone_number" placeholder="Contoh: 081234567890" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-biotech-secondary/50 focus:border-biotech-secondary transition">
                    </div>

                    <div class="md:col-span-2">
                        <label for="office_room" class="block text-sm font-semibold text-gray-600 mb-2">Kantor (Ruangan)</label>
                        <input type="text" name="office_room" id="office_room" placeholder="Contoh: Gedung 1, Lantai 2, R. Biotek" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-biotech-secondary/50 focus:border-biotech-secondary transition" required>
                    </div>
                        ...
                        <div class="overflow-x-auto shadow border border-gray-200 rounded-xl bg-white p-4">
                            <table class="min-w-full divide-y divide-gray-200" id="tabel-pengajaran">
                                ...
                                <tbody class="divide-y divide-gray-200" id="wrapper-pengajaran">
                                    @if(isset($lecturer) && $lecturer->pengajarans->count() > 0)
                                        @foreach($lecturer->pengajarans as $index => $ajar)
                                            <tr>
                                                <td class="p-2">
                                                    <input type="text" name="course_name[]" value="{{ $ajar->course_name }}" required class="w-full px-3 py-2 border rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500">
                                                </td>
                                    ...
                                    @else
                                        <tr>
                                            <td class="p-2">
                                                <input type="text" name="course_name[]" required placeholder="Nama Mata Kuliah" class="w-full px-3 py-2 border rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500">
                                            </td>
                                            <td class="p-2 text-center">
                                                <button type="button" class="btn-hapus-baris bg-red-100 hover:bg-red-200 text-red-600 p-2 rounded-lg transition">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                        ...
                        document.addEventListener("DOMContentLoaded", function() {
                            ...
                            const barisBaru = `
                                <tr>
                                    ...
                                    <td class="p-2">
                                        <input type="text" name="academic_year[]" required placeholder="Contoh: 2025/2026" class="w-full px-3 py-2 border rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500">
                                    </td>
                                    <td class="p-2 text-center">
                                        <button type="button" class="btn-hapus-baris bg-red-100 hover:bg-red-200 text-red-600 p-2 rounded-lg transition">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
