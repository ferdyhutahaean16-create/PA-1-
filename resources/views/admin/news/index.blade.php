...
<div class="py-10 bg-gray-50 min-h-screen">
    <div class="container mx-auto px-6">
        <div class="flex justify-between items-center mb-8">
            ...
            <a href="{{ route('news.create') }}" class="bg-emerald-600 text-white px-6 py-2.5 rounded-lg font-bold shadow hover:bg-emerald-500 transition flex items-center gap-2">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="p-4 text-center">
                                @if($newsItem->image)
                                    <img src="{{ asset($newsItem->image) }}" class="w-20 h-14 object-cover rounded shadow-sm mx-auto">
                                @else
                                    <div class="w-20 h-14 bg-gray-200 rounded flex items-center justify-center text-xs text-gray-400 mx-auto">No Image</div>
                                @endif
                            </td>
                            <td class="p-4">
                                <div class="font-bold text-gray-800 line-clamp-1">{{ $newsItem->title }}</div>
                                <div class="text-xs text-gray-500 mt-1 line-clamp-1">
                                    {{ Str::limit(strip_tags($newsItem->content), 60) }}
                                </div>
                            </td>
                            <td class="p-4 text-center text-sm text-gray-600">
                                {{ \Carbon\Carbon::parse($newsItem->published_date)->format('d M Y') }}
                            </td>
                            <td class="p-4 text-center text-sm">
                                <span class="bg-blue-100 text-blue-700 py-1 px-3 rounded-full text-xs font-bold">{{ $newsItem->views }}</span>
                            </td>
                            <td class="p-4 text-center flex justify-center gap-2 mt-2">
                                <a href="{{ route('news.edit', $newsItem->id) }}" class="bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-1.5 rounded text-xs transition">Edit</a>
                                <form action="{{ route('news.destroy', $newsItem->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus berita ini?');">
                                    @csrf
                                    @method('DELETE')
