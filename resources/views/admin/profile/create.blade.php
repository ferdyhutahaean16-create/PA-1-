...
<div class="py-10 bg-gray-50 min-h-screen">
    <div class="container mx-auto px-6 max-w-5xl">
        ...
        <div class="mb-8">
            <a href="{{ route('profile.index') }}" class="text-emerald-600 hover:text-emerald-500 flex items-center gap-2 mb-4 font-semibold">
            ...
            <form action="{{ route('profile.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div id="konten-teks" class="block space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="history" rows="5" class="ckeditor-field w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1a4a38]">{{ old('history') }}</textarea>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Visi <span class="text-red-500">*</span></label>
                            <textarea name="vision" rows="4" class="ckeditor-field w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1a4a38]">{{ old('vision') }}</textarea>
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Misi <span class="text-red-500">*</span></label>
                            <textarea name="mission" rows="4" class="ckeditor-field w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1a4a38]">{{ old('mission') }}</textarea>
                        </div>
                    </div>

                    <div class="hidden"> 
                        <label class="block text-sm font-bold text-gray-700 mb-2">Tujuan</label>
                        <textarea name="goals" rows="2" class="w-full p-3 border border-gray-300 rounded-lg">-</textarea>
                    </div>

                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">Prospek Karir <span class="text-red-500">*</span></label>
                        <textarea name="career_prospects" rows="4" class="ckeditor-field w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1a4a38]">{{ old('career_prospects') }}</textarea>
                    </div>
                </div>
