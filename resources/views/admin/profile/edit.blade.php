...
<div class="py-10 bg-gray-50 min-h-screen">
    <div class="container mx-auto px-6 max-w-5xl">
        ...
        <div class="mb-8">
            <a href="{{ route('profile.index') }}" class="text-emerald-600 hover:text-emerald-500 hover:underline flex items-center gap-2 mb-4 font-semibold">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
                Kembali
            </a>
            <h1 class="text-3xl font-bold text-gray-800">Kelola Data Profil Umum</h1>
        </div>
            ...
            <form action="{{ route('profile.update', $profile->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div id="konten-teks" class="block space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="history" rows="5" class="ckeditor-field w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">{{ $profile->history }}</textarea>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Visi</label>
                            <textarea name="vision" rows="4" class="ckeditor-field w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">{{ $profile->vision }}</textarea>
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Misi</label>
                            <textarea name="mission" rows="4" class="ckeditor-field w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">{{ $profile->mision }}</textarea>
                        </div>
                    </div>

                    <div class="hidden"> <label class="block text-sm font-bold text-gray-700 mb-2">Tujuan</label>
                        <textarea name="goals" rows="2" class="ckeditor-field w-full p-3 border border-gray-300 rounded-lg">{{ $profile->goals ?? '-' }}</textarea>
                    </div>

                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">Prospek Karir</label>
                        <textarea name="career_prospects" rows="4" class="ckeditor-field w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">{{ $profile->career_prospects }}</textarea>
                    </div>
                </div>
