...
<div class="py-10 min-h-screen bg-gray-50/50">
    <div class="container mx-auto px-6">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            ...
            <a href="{{ route('ruang-kelas.create') }}" class="bg-green-600 hover:bg-green-700 text-white font-bold py-2.5 px-5 rounded-lg shadow-md transition flex items-center gap-2 text-sm">
                ...
                <table class="w-full text-left border-collapse">
                    <thead class="bg-gray-800 text-white text-sm uppercase tracking-wider">
                        <tr>
                            ...
                            <th class="p-4 font-semibold">Nama Ruangan & Fasilitas</th>
                    ...
                    <tbody class="divide-y divide-gray-200 text-gray-700">
                        @forelse($ruang_kelas as $index => $item)
                        <tr class="hover:bg-gray-50 transition duration-150">
                            <td class="p-4 text-center font-bold text-gray-500">{{ $index + 1 }}</td>
                            <td class="p-4">
                                <div class="text-base font-bold text-gray-900">{{ $item->nama_ruangan }}</div>
                                @if($item->fasilitas_pendukung)
                            ...
                            <td class="p-4 text-center">
                                @if($item->foto)
                                    <img src="{{ asset($item->foto) }}" class="w-16 h-12 object-cover rounded border shadow-sm mx-auto hover:scale-150 transition-transform cursor-pointer">
                                @else
                            ...
                            <td class="p-4 text-center">
                                <div class="flex justify-center gap-2">
                                    <a href="{{ route('ruang-kelas.edit', $item->id) }}" class="bg-yellow-500 text-white px-3 py-1.5 rounded-md text-sm font-semibold hover:bg-yellow-600 transition shadow-sm">Edit</a>
                                    <form action="{{ route('ruang-kelas.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Hapus ruangan ini secara permanen?')">
                                        @csrf @method('DELETE')
