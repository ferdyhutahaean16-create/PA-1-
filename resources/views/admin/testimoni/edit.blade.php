...
<div class="py-10 bg-gray-50 min-h-screen">
    <div class="container mx-auto px-6 max-w-4xl">
        <div class="mb-8 flex items-center justify-between">
            ...
            <a href="{{ route('testimoni.index') }}" class="text-gray-500 hover:text-gray-800 font-bold flex items-center gap-2">
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
            <form action="{{ route('testimoni.update', $testimonial->id) }}" method="POST" enctype="multipart/form-data" class="p-8">
                @csrf
                @method('PUT')
                ...
                <div class="mb-6">
                    ...
                    <textarea name="testimony" rows="5" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-[#1a4a38]" required>{{ $testimonial->testimony }}</textarea>
                </div>

                <div class="flex justify-end">
                    <button type="submit" class="bg-yellow-500 text-white px-8 py-3 rounded-lg font-bold shadow-md hover:bg-yellow-600 transition">Update Testimoni</button>
                </div>
            </form>
        </div>
    </div>
