...
<div class="bg-gray-50 py-12 border-b border-gray-200">
    <div class="container mx-auto px-6 text-center">
        ...
        <p class="text-gray-600">Pencapaian kompetisi</p>
    </div>
</div>
...
<div class="container mx-auto px-6 py-12 max-w-6xl">
    ...
    <div class="flex justify-center gap-4 mb-16 overflow-x-auto pb-4">
        <button class="bg-[#1a4a38] text-white px-6 py-2.5 rounded-full font-semibold text-sm shadow-md whitespace-nowrap cursor-default">
            Prestasi & Lomba
        </button>
    </div>
            ...
            <div class="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden relative flex flex-col transition-transform hover:-translate-y-1 hover:shadow-lg">
                ...
                <div class="p-6 pt-8 flex-1 flex flex-col">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <span class="text-sm text-gray-400 font-medium">{{ $item->tahun ?? \Carbon\Carbon::parse($item->tanggal_perolehan)->format('Y') }}</span>
                    </div>

                    <h3 class="text-lg font-bold text-gray-900 mb-2 leading-snug">
                        {{ $item->judul_prestasi ?? $item->nama_prestasi }}
                    </h3>
                    ...
                    <div class="border-t border-gray-200 pt-4 flex items-center gap-3 mt-auto">
                        @if($item->foto || $item->bukti_sertifikat)
                            <img src="{{ asset($item->bukti_sertifikat ?? $item->foto) }}" alt="{{ $item->nama_peraih }}" class="w-10 h-10 rounded-full object-cover border border-gray-200">
                        @else
